...
const FontLoader: React.FC = () => (
  <style>{`
    ...
    @keyframes nasdaqBgUp { 0%, 40% { background-color: #00FF00; } 100% { background-color: transparent; } }
    @keyframes nasdaqTextUp { 0%, 40% { color: #000000; } 100% { color: var(--accent-color); } }
    @keyframes nasdaqBgDown { 0%, 40% { background-color: #FF0000; } 100% { background-color: transparent; } }
    @keyframes nasdaqTextDown { 0%, 40% { color: #FFFFFF; } 100% { color: var(--accent-color); } }
    ...
    @keyframes metalShowcase { 0% { opacity: 0; transform: scale(0.99); } 6% { opacity: 1; transform: scale(1); } 25% { opacity: 1; transform: scale(1); } 31% { opacity: 0; transform: scale(1.01); } 100% { opacity: 0; } }
    @keyframes tickerScroll { 0% { transform: translateX(0%); } 100% { transform: translateX(-50%); } }
    .ticker-track-slow { display: inline-flex; white-space: nowrap; align-items: center; animation: tickerScroll 30s linear infinite; }
  `}</style>
);

const DASHBOARD = {
  goldBright: "#FFC233", silverBright: "#B9DBF5", text: "#FFFFFF", textMuted: "#A6ADB8", green: "#00FF00", red: "#FF0000", date: "#D9B75F",
};

interface RateQuote {
  ask: number | string;
  bid: number | string;
  high_ask?: number | string;
  low_bid?: number | string;
  [key: string]: any;
}
...
const usePriceDelta = (current: number | string | undefined): PriceDelta | null => {
  const prevRef = useRef<number | undefined>(undefined);
  const [delta, setDelta] = useState<PriceDelta | null>(null);
  useEffect(() => {
    if (current === undefined) return;
    const curNum = typeof current === "string" ? parseFloat(current) : current;
    if (Number.isNaN(curNum)) return;
    const prev = prevRef.current;
    if (prev !== undefined && prev !== curNum) {
...
const DataTable: React.FC<{ rates: MetalRates | null; loading: boolean }> = ({ rates, loading }) => {
  ...
  const formatPrice = (n: number | string | undefined) => {
    if (n === undefined || n === null || n === "") return "";
    const num = Number(n);
    return !Number.isNaN(num) ? num.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : "";
  };
  ...
  return (
    <div className="w-full h-full flex flex-col justify-end rounded-2xl overflow-hidden border border-white/10 shadow-2xl bg-[#0a0e14]">
      <div className="flex items-center px-[2vw] shrink-0" style={{ background: "linear-gradient(90deg, #D3A83B 0%, #F8E78A 100%)", height: "6.5vh" }}>
        <div className="grid grid-cols-[1.6fr_0.8fr_1.7fr_1.7fr] gap-[1vw] w-full">
          <div className="font-extrabold text-[#050505]" style={{ fontSize: `${1.0 * fontScale}rem` }}>METAL</div>
          <div className="font-extrabold text-[#050505] text-center" style={{ fontSize: `${1.0 * fontScale}rem` }}>WEIGHT</div>
          <div className="font-extrabold text-[#050505] text-left" style={{ fontSize: `${1.0 * fontScale}rem` }}>BID (AED)</div>
          <div className="font-extrabold text-[#050505] text-left" style={{ fontSize: `${1.0 * fontScale}rem` }}>ASK (AED)</div>
        </div>
      </div>
            ...
            <div key={index} className={`flex-1 flex items-center px-[2vw] ${index !== tableData.length - 1 ? 'border-b border-white/5' : ''}`}>
              <div className="grid grid-cols-[1.6fr_0.8fr_1.7fr_1.7fr] gap-[1vw] items-center w-full min-w-0">
                <div className="font-bold text-white whitespace-nowrap overflow-hidden text-ellipsis" style={{ fontSize: `${1.0 * fontScale}rem` }}>
                  {item.label} <span style={{ color: DASHBOARD.goldBright, fontSize: `${0.85 * fontScale}rem` }}>{item.purity}</span>
                </div>
                <div className="font-bold text-center text-white" style={{ fontSize: `${1.0 * fontScale}rem` }}>{item.weight}</div>
                <div className="font-extrabold text-left whitespace-nowrap overflow-hidden" style={{ color: DASHBOARD.goldBright, fontSize: `${1.4 * fontScale}rem`, fontFamily: "'Inter', sans-serif", fontVariantNumeric: "tabular-nums" }}>
...
// DEEP SEARCH ALGORITHM: Aggressively hunts for high/low nested inside the API quote object
const extractNestedValue = (obj: any, target: 'high' | 'low'): number | undefined => {
  if (!obj || typeof obj !== 'object') return undefined;

  // 1. Direct hit on primary known API keys
  const exactKeys = target === 'high' ? ['high_ask', 'highask', 'high', 'dayhigh'] : ['low_bid', 'lowbid', 'low', 'daylow'];
  ...
  // 2. Broad shallow scan
  for (const [key, value] of Object.entries(obj)) {
    if (key.toLowerCase().includes(target) && value !== null && value !== "") {
      const val = parseFloat(value as string);
      if (!Number.isNaN(val) && val > 0) return val;
    }
  }

  // 3. Deep recursion for nested objects (e.g. quote.details.high_ask)
  for (const value of Object.values(obj)) {
    if (typeof value === 'object') {
      const nestedResult = extractNestedValue(value, target);
...
const MetalSpotCard: React.FC<{ metal: "gold" | "silver"; quote?: RateQuote; loading: boolean }> = ({ metal, quote, loading }) => {
  ...
  // Diagnostics: View the exact API payload in your browser console
  useEffect(() => {
    if (quote) console.log(`[API Diagnostic - ${metal.toUpperCase()}] Raw Quote Object:`, quote);
  }, [quote, metal]);
  ...
  return (
    <section className="bg-[#0a0e14] rounded-2xl p-[1.5vw] flex flex-col justify-between relative shadow-2xl w-full flex-1 min-h-0 overflow-hidden" style={{ border: `2px solid ${accent}` }}>
      ...
      <div className="flex w-full flex-1 items-center">
        {(["BID","ASK"] as const).map((side) => {
          const rawPrice = side === "BID" ? quote?.bid : quote?.ask;
          ...
          return (
            <div key={side} className="flex flex-col items-center w-[50%] min-w-0">
              ...
              <div className={`mt-[0.5vh] w-[90%] h-[6vh] rounded-xl flex items-center justify-center transition-colors ${delta?.direction === "up" ? "flash-bg-up" : delta?.direction === "down" ? "flash-bg-down" : ""}`}>
                <span className={`font-extrabold ${delta?.direction === "up" ? "flash-text-up" : delta?.direction === "down" ? "flash-text-down" : ""}`} style={{ '--accent-color': accent, color: "var(--accent-color)", fontSize: `${2.0 * fontScale}rem`, fontFamily: "'Inter', sans-serif", fontVariantNumeric: "tabular-nums" } as React.CSSProperties}>
                  {!loading && price !== undefined && !Number.isNaN(price) ? price.toFixed(decimals) : "..."}
                </span>
              </div>
            </div>
          );
...
const AuthenticatedHome: React.FC = () => {
  ...
  return (
    <div className="flex flex-col w-screen h-screen px-[2vw] py-[2vh] text-white overflow-hidden bg-[#080808] box-border" style={{ fontFamily: "Manrope, ui-sans-serif, system-ui, sans-serif" }}>
      <FontLoader />
      <div className="absolute inset-0 z-0" style={{ backgroundImage: `linear-gradient(90deg, rgba(0,0,0,0.6), rgba(0,0,0,0.3)), url(${ASSETS.background})`, backgroundSize: "cover", backgroundPosition: "center" }} />
      <div className="relative z-10 flex flex-col w-full h-full justify-between">
        <header className="flex w-full justify-between items-center" style={{ height: "22vh" }}>
          <div className="w-[58%] h-full relative">
            <div className="absolute left-0 top-0 bottom-0 flex flex-col justify-center border-r border-white/20 pr-[1.5vw] z-20">
              ...
              <div className="font-normal uppercase leading-tight mb-[0.3vh]" style={{ fontFamily: "'Barlow Condensed', sans-serif", color: DASHBOARD.date, fontSize: `${1.1 * fontScale}rem` }}>{currentDate.toLocaleDateString("en-US", { timeZone: "Asia/Dubai", weekday: "long" })}</div>
              <div className="font-extrabold uppercase leading-none" style={{ fontFamily: "'Barlow Condensed', sans-serif", color: DASHBOARD.date, fontSize: `${1.4 * fontScale}rem` }}>{currentDate.toLocaleDateString("en-US", { timeZone: "Asia/Dubai", month: "short", day: "2-digit", year: "numeric" })}</div>
            </div>
            <div className="absolute inset-0 flex items-center justify-center z-10 pointer-events-none">
              <img src={logoSrc} alt="Company Logo" className="w-auto object-contain" style={{ height: "19vh", maxWidth: "80%" }} />
            </div>
          </div>
        ...
        <footer className="mt-[2vh] flex items-center px-[2vw] border-t border-white/10 bg-[#000000]/80 rounded-2xl w-full shadow-2xl shrink-0" style={{ height: "7vh" }}>
          ...
          <div className="w-px h-[3vh] bg-white/20 mx-[2vw] shrink-0" />
          <span className="font-extrabold uppercase" style={{ fontFamily: "'Barlow Condensed', sans-serif", color: DASHBOARD.goldBright, fontSize: `${1.1 * fontScale}rem` }}>{uaeTime}</span>
        </footer>
      </div>
    </div>
  );
